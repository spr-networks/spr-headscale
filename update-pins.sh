...
# lookups (registry manifest inspect, go.dev, GitHub). Review with `git diff`.
...
# Latest stable headscale release tag (GitHub marks pre-releases; /latest skips them),
# pinned by the tag's full commit hash (annotated tags are peeled via ^{}).
HEADSCALE_VERSION=$(curl -fsSL "https://api.github.com/repos/${HEADSCALE_REPO}/releases/latest" \
  | python3 -c 'import json,sys; print(json.load(sys.stdin)["tag_name"])')
HEADSCALE_COMMIT=$(git ls-remote "https://github.com/${HEADSCALE_REPO}.git" \
  "refs/tags/${HEADSCALE_VERSION}" "refs/tags/${HEADSCALE_VERSION}^{}" | tail -1 | cut -f1)
[ -n "$HEADSCALE_COMMIT" ] || { echo "could not resolve headscale tag ${HEADSCALE_VERSION}" >&2; exit 1; }
...
HEADSCALE_COMMIT=${HEADSCALE_COMMIT}
EOF
